...
function saveName(name) {
    ...
    document.cookie =
        'prefs=' + escape(prefsString) + '; expires=' + expDate.toUTCString();
    ...
    document.getElementById('greeting').innerHTML =
        '<a class="bright" target="_self" href="javascript:getNameAgain()" title="Click here if you\'re not ' +
        prefs.name +
        '">Hello ' +
        prefs.name +
        '!</a><br>';
...
function redirect(destination) {
    ...
    // external page
    if (
        destination.indexOf('monospace') == -1 &&
        ...
        if (!userPrefs) {
            ...
            if (prefs.linkOptions == 'same') {
                ...
                window.open(
                    destination,
                    '',
                    'favorites=yes,toolbar=yes,location=yes,status=yes,directories=yes,menubar=yes,scrollbars=yes,resizable=yes'
                );
            }
        }
...
function welcome() {
    if (userPrefs) {
        if (prefs.name != '') {
            ...
            markup =
                '<a class="bright" target="_self" href="javascript:getName()" title="Click to enter your name">Hello! What is your name?</a><br>';
        }
...
function popDutch() {
    pop_scroll('dutchwordlist.php', 'dutchwordlist', 700, 500);
}
...
function colorCycle(layer) {
    ...
    if (color == 'red')
        document.all[layer].style.color = 'ff' + hexColor + hexColor;
    else if (color == 'blue')
        document.all[layer].style.color = hexColor + hexColor + 'ff';
...
function calendar(month, day, year) {
    ...
    var html =
        '<div class="link"><div class="category">' +
        calMonths[month - 1][0] +
        ' ' +
        year +
        '</div><table border="0" cellpadding="0" cellspacing="0" class="calendarTable"><tr>';
    for (i = 0; i < calDays.length; i++)
        html +=
            '<td class="daysLabel" width="20">' +
            calDays[i].substring(0, 3) +
            '</td>';
    for (row = 0; row < numRows; row++) {
        ...
        for (col = 0; col < 7; col++) {
            ...
            if (counter - startDay > calMonths[month - 1][1])
                html += '<td class="daysInactive">&nbsp;</td>';
            else if (counter > startDay)
                html +=
                    '<td class="days' +
                    addClass +
                    '">' +
    ...
    html +=
        '<div><span class="gray">Time in New York: </span><span id="nycTime"></span></div>';
    html +=
        '<div><span class="gray">Your Local Time: </span><span id="localTime"></span></div>';
    html += '</div></div>';
